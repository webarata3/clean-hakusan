...
import Html.Attributes exposing (attribute, class, classList, for, href, id, selected, src, target, type_, value)
...
viewHeader =
    Html.header [ class "header" ]
        ...
        , h1 [ class "header__title" ] [ text "白山市ごみ収集日程" ]
        , div [ class "main__menu-button" ]
            [ button
                [ class "header__reload-button"
            ...
            [ li []
                [ a
                    ...
                    , img
                        ...
                        , class "submenu__external-link-image"
                        ]
                        []
                    ]
                ]
...
viewMenuClass model =
    ...
    case model.menuState of
        MenuClose ->
            class ("menu__close" ++ appendClass)

        MenuOpen ->
            class ("menu__open" ++ appendClass)
...
viewMain model =
    ...
    case model.viewState of
        SystemError ->
            main_
                [ class "main error" ]
                [ div [ class "error__message" ] [ text "エラーが発生しました。" ]
                , div [ class "error__message" ] [ text "動かない場合には、再読み込みしてみてください。" ]
                , div [ class "error__message" ] [ text "報告して頂ける場合には、下の理由をお知らせください。" ]
        ...
        PrepareData ->
            main_
                ...
                , div []
                    [ img [ class "loading__icon", src "image/ball-triangle.svg" ] [] ]
                ]
        ...
        DataOk ->
            case model.maybeAreaGarbage of
                Just areaGarbage ->
                    main_ [ class "main" ]
                        [ div [ class "main__alert" ] [ text "※ 白山市公式のアプリではありません。" ]
                        , div [ class "area" ]
                            [ div [ class "area__select" ]
                                [ label
                                    [ for "area", class "main__label" ]
                                    [ text "地域" ]
                                , select
                                    [ id "area"
                                    , class "main__select"
...
viewGarbages currentDate garbages =
    ...
    in
    div [ class "garbage" ] (List.map viewGarbage2 garbages)


viewGarbage : String -> Garbage -> Html Msg
viewGarbage currentDate garbage =
    div [ class "garbage__item" ]
...
viewGarbageTitles garbageTitles =
    div [ class "garbage__title" ] (List.map viewLine garbageTitles)
...
viewGarbageDates currentDate garbageDates =
    ...
    div [ class (Util.howManyDaysCss howManyDays) ]
        [ div [ class "garbage__how-many-days" ] [ text dispDays ]
        , div [ class "garbage__next-date" ]
...
viewSubMenuCredit isOpen =
    div
        ...
        [ div [ class "submenu__window credit" ] <|
...
viewCredit credit =
    div [ class "credit__license" ]
        [ h3 [ class "credit__license-title" ]
            [ a
                [ href credit.link ]
                [ text credit.title ]
            ]
        , div [] [ pre [ class "credit__pre" ] [ text credit.license ] ]
        ]
